...
# ─── Helpers ──────────────────────────────────────────────────────────────────
def add_header(doc, logo_bytes, ncc=""):
    ...
    if ncc:
        ...
        run2 = p2.add_run(f"NHÀ CUNG CẤP: {ncc}")
        run2.bold = True
        run2.font.size = Pt(20)
        run2.font.name = "Arial"
        run2.font.color.rgb = RGBColor(220, 0, 0)
...
def make_ma_phieu(dt):
    return f"PXK-{dt.strftime('%Y%m%d%H%M%S')}"

def add_footer(doc, ncc="", show_pmh=False):
    ...
    text_footer = f"Phòng Mua Hàng | {ncc}" if show_pmh else ncc
    r1 = para.add_run(text_footer)
    r1.font.size = Pt(9)
    r1.font.name = "Arial"
            ...
            if st.button("🖨️ Tạo phiếu xuất kho", type="primary", use_container_width=True):
                ...
                with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
                    ...
                    for i, (sheet_name, branch, items) in enumerate(tasks):
                        ...
                        # Tên file: NCC_Tên Trung Tâm_Mã phiếuif uploaded:
                        ...
                        safe_ncc_name = ncc_name.replace(" ", "-").replace("/", "-")
                        safe_branch_name = branch.strip().replace("/", "-").replace("\\", "-")
                        filename = f"[TT-{safe_branch_name}] NCC-{safe_ncc_name}_{ma_phieu}.docx"
...
# ─── Tab 2: Phiếu chuyển hàng ────────────────────────────────────────────────
    ...
    CHI_NHANH_CHUYEN_MAC_DINH = "KHO TỔNG"
    ...
    if uploaded_chuyen:
        ...
        with st.spinner("Đang đọc file Excel..."):
            try:
                all_branches_chuyen = parse_excel_raw(uploaded_chuyen.read())
                st.session_state["data_chuyen_hang"] = all_branches_chuyen
            except Exception as e:
                st.error(f"Lỗi đọc file: {e}")
                st.stop()

        if not all_branches_chuyen:
            st.warning("Không tìm thấy trung tâm nào có số lượng > 0.")
        else:
            st.success(f"Tìm thấy **{len(all_branches_chuyen)} trung tâm** có hàng")
            ...
            if selected_ch and st.button("🖨️ Tạo phiếu chuyển hàng từ Excel", type="primary", use_container_width=True):
                ...
                zip_buf_ch = io.BytesIO()
                count_ch = 0

                with zipfile.ZipFile(zip_buf_ch, "w", zipfile.ZIP_DEFLATED) as zf:
                    ...
                    for i, branch in enumerate(selected_ch):
                        ...
                        ma_phieu_ch = f"PCH-{now_ch.strftime('%Y%m%d')}-{now_ch.strftime('%H%M%S')}"
                ...
                st.download_button(
                    label=f"⬇️ Tải về {chuyen_basename}.zip",
                    data=zip_buf_ch,
                    file_name=f"{chuyen_basename}.zip",
                    mime="application/zip",
                    use_container_width=True,
                )
    ...
    # ── Phần 2: Nhập tay ──────────────────────────────────────────────────────
    st.markdown("#### ✏️ Tạo phiếu nhập tay")
    st.caption("Nhập từng dòng: Mã hàng | Tên hàng | ĐVT | SL chuyển")
...
# CHUYỂN ĐỔI ĐỊNH DẠNG PHIẾU HÀNG KID
    ...
    if uploaded_hpc:
        # ── Đọc file ──────────────────────────────────────────────────────
        import openpyxl
        wb_raw = openpyxl.load_workbook(io.BytesIO(uploaded_hpc.read()), data_only=True)
        ws_raw = wb_raw.active
        rows_raw = list(ws_raw.iter_rows(values_only=True))

        row1 = rows_raw[0]  # tên trung tâm
        row2 = rows_raw[1]  # sub-header
...
# CHUYỂN ĐỔI ĐỊNH DẠNG PHIẾU HÀNG GAME
    ...
    if uploaded_game:
        wb_raw = openpyxl.load_workbook(io.BytesIO(uploaded_game.read()), data_only=True)
        ws_raw = wb_raw.active
        ...
        for i, val in enumerate(row2):
            ...
            col_1idx = i + 1
            # Tìm cột Số đặt gần nhất sau cột TT này
            nearest_so_dat = next((c for c in so_dat_cols if c >= col_1idx), None)
            if nearest_so_dat is None:
                continue
        ...
        for row_idx in range(5, ws_raw.max_row + 1):
            ...
            dvt = str(ws_raw.cell(row_idx, COL_DVT).value or "").strip()
            try:
