...
# ─── Tạo một phiếu Word ───────────────────────────────────────────────────────
def build_phieu(branch_name, items, now, logo_bytes=None, ghi_chu="Upload lên Link HPC (Không Kiot)"):
    doc = Document()
...
def parse_excel_raw(file_bytes):
    ...
    for sheet_name in wb.sheetnames:
        ...
        row2 = rows[1]  # Hàng 2: tên chi nhánh
        row3 = rows[2]  # Hàng 3: sub-header chi tiết

        # ── Tìm cột cố định: scan cả hàng 2 lẫn hàng 3, lấy kết quả đầu tiên ──
        def find_col(keywords, search_limit=20):
            for row in (row3, row2):
                for i in range(0, min(search_limit, len(row))):
                    if row[i] and any(kw.lower() in str(row[i]).lower() for kw in keywords):
                        return i
            return None

        COL_NCC       = find_col(["NCC"])
        COL_TEN_HANG  = find_col(["Tên hàng"])
        COL_CACH_DONG = find_col(["Đơn vị đặt hàng"])
        COL_GIA_COST  = find_col(["Đơn giá/ Cái", "Đơn giá/Cái"])

        # Fallback số cố định nếu không tìm thấy
        if COL_NCC       is None: COL_NCC       = 4
        if COL_TEN_HANG  is None: COL_TEN_HANG  = 6
        if COL_CACH_DONG is None: COL_CACH_DONG = 10
        if COL_GIA_COST  is None: COL_GIA_COST  = 11

        # ── Tìm chi nhánh từ hàng 2, map sang cột "Số đặt" ở hàng 3 ────────
        branch_list_final = []
        for i in range(14, len(row2)):
            ...
            if not val:
                continue
            if 'tháng' in val.lower():
                continue
            try:
                float(val)
                continue  # bỏ qua số như 1.5
            except ValueError:
                pass
            # Scan hàng 3 từ vị trí này tìm "Số đặt"
            col_so_dat = None
        ...
        for branch_name, col_so_dat in branch_list_final:
            ...
            for row in rows[3:]:
                ...
                try:
                    gia_cost = row[COL_GIA_COST] if row[COL_GIA_COST] else 0
                except (ValueError, TypeError):
                    gia_cost = 0
                ...
                items.append({
                    ...
                    "Tên Hàng":  ten_hang,
                    "Cách đóng": str(row[COL_CACH_DONG]).strip() if COL_CACH_DONG < len(row) and row[COL_CACH_DONG] else "",
                    "Giá cost":  gia_cost,
                    "Số đặt":    int(so_dat),
                })
    ...
    if st.button("🖨️ Tạo phiếu xuất kho", type="primary", use_container_width=True):
        ...
        with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for i, (sheet_name, branch, items) in enumerate(tasks):
                docx_bytes = build_phieu(branch, items, now, logo_bytes, ghi_chu_option)
